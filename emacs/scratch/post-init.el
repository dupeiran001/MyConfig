...
;; of your `~/.config/emcas/scratch/post-init.el` file, before all other packages.
...
;; One-ESC to quit (prompts, minibuffer, help buffers, etc.)
...
(use-package treemacs-evil
  ...
  (define-key evil-treemacs-state-map (kbd "<escape>") #'treemacs-select-window)
  (define-key evil-treemacs-state-map (kbd ".") #'treemacs-select-directory)
  ...
  (define-key evil-treemacs-state-map (kbd "m") #'treemacs-move-file)
  (define-key evil-treemacs-state-map (kbd "b") #'treemacs-bookmark))
...
;; Easy movement to any visible text
(use-package avy
  :ensure t
...
;; Set the default font to Cascadia Code with specific size and weight
(set-face-attribute 'default nil
                    :height 120 :weight 'normal :family "CaskaydiaCove Nerd Font")
