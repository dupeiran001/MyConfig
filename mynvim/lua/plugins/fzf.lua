return {
  ...
  keys = {
    ...
    { "<leader>fm",  function() _G.fzf_marks() end,                                  desc = "Bookmarks" },
    ...
    _G.fzf_marks = function(opts)
      ...
      local function contents(callback)
        for _, mark in ipairs(marklist) do
          local entry = string.format("%s%s %s", utils.ansi_codes.magenta(mark.filename),
            utils.ansi_codes.green(":" .. tostring(mark.lnum) .. ":"), mark.text)
          callback(entry)
        end
